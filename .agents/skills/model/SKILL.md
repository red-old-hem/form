---
description: Create or update a database table, its validators, and its server model, including changes to existing table-backed fields and behavior.
---
...
**Naming:** The JS export name must match the SQL table name exactly. `createModel` uses `getTableName()` (which returns the SQL name) to look up the table in `db.query` (which is keyed by JS export name). If they differ, the model will fail at runtime with "Model {name} not found". Use `snake_case` for both: `export const demo_forms = sqliteTable('demo_forms', ...)`.
